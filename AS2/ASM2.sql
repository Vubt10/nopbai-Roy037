...
-- 1. Tạo Login (Authentication SQL Server)
CREATE LOGIN [Admin_QLNT] WITH PASSWORD=N'Ad@min123', DEFAULT_DATABASE=[QLNHATRO_NGUYENHOANGMINH], CHECK_EXPIRATION=OFF, CHECK_POLICY=OFF;
CREATE LOGIN [User_QLNT] WITH PASSWORD=N'Us@er123', DEFAULT_DATABASE=[QLNHATRO_NGUYENHOANGMINH], CHECK_EXPIRATION=OFF, CHECK_POLICY=OFF;
GO

-- CHUYỂN SANG CSDL CỦA BẠN ĐỂ TẠO USER VÀ PHÂN QUYỀN
USE QLNHATRO_NGUYENHOANGMINH;
GO

-- 2. Tạo User trong CSDL
CREATE USER [Admin_QLNT] FOR LOGIN [Admin_QLNT];
CREATE USER [User_QLNT] FOR LOGIN [User_QLNT];
GO

-- 3. Phân quyền

-- 3.1. Người dùng thứ nhất: Quản trị CSDL (DB Administrator)
-- Gán quyền toàn quyền trên CSDL
EXEC sp_addrolemember 'db_owner', 'Admin_QLNT';
GO

-- 3.2. Người dùng thứ hai: Người dùng thông thường (Standard User)
-- Quyền thao tác trên các bảng (SELECT, INSERT, UPDATE, DELETE)
GRANT SELECT, INSERT, UPDATE, DELETE ON LOAINHA TO User_QLNT;
GRANT SELECT, INSERT, UPDATE, DELETE ON NGUOIDUNG TO User_QLNT;
GRANT SELECT, INSERT, UPDATE, DELETE ON NHATRO TO User_QLNT;
GRANT SELECT, INSERT, UPDATE, DELETE ON DANHGIA TO User_QLNT;

-- Quyền thực thi các SP và Hàm
GRANT EXECUTE ON SCHEMA::dbo TO User_QLNT; -- Cấp quyền thực thi cho tất cả SP/Hàm trong Schema dbo
GO
